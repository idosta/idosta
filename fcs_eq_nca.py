...
def eq_nca(v, eps, u, temperature, t_m, t_l, t_max, lamb):
    ga = (t_m ** 2) / t_l
    ...
    N = 10 * t_max  # number of time data points
    ...
    plt.plot(times_plus, G[0])
    plt.plot(times_plus, G[1])
    plt.show()
    ...
    while delta_K > d_dyson:
        ...
        C += 1
    plt.plot(times_plus, K[0])
    plt.plot(times_plus, K[1])
    plt.show()
    ...
    def current_cal(vertex, propagator, hyb_g):
        ...
        return fft_integral(down, propagator[0][: N // 2 + 1]) + fft_integral(empt, propagator[1][: N // 2 + 1])
    # plt.plot(times[N // 2:], current_cal(K, hG, G))
    # plt.show()

    return K[:, N // 2]


y = eq_nca(1, 0, 1, 1, 1, 10, 20, 0)
print(y)


# cant simply add counting field due to normalization